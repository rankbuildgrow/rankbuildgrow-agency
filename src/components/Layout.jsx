...
import { Menu, X, ArrowUp, CheckCircle2 } from "lucide-react";
    ...
    <header className="fixed top-0 w-full z-50 h-[72px] bg-primary/95 backdrop-blur-[20px] border-b border-white/5">
      <div className="max-w-[1200px] mx-auto px-5 md:px-10 lg:px-[60px] h-full flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3">
          <img src="/images/logo.png" alt="RankBuildGrow Logo" className="h-10 w-10 object-contain bg-mint/10 rounded-lg" />
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {links.map((link) => (
            <Link key={link.name} to={link.path} className={`font-inter text-[13px] transition-colors duration-200 ${location.pathname === link.path ? "text-[#F5F5F5]" : "text-[#707070] hover:text-mint"}`}>
              {link.name}
            </Link>
          ))}
          <Button variant="nav" href="/contact">Get Free Quote</Button>
        </nav>
        <button className="lg:hidden text-chrome" onClick={() => setIsOpen(!isOpen)}>
    ...
    <div className="max-w-[1200px] mx-auto px-5 md:px-10 lg:px-[60px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-16">
        <div>
          ...
          <ul className="space-y-3 font-inter text-[14px] text-[#707070]">
      ...
      <div className="border-t border-white/5 pt-8 text-center font-inter text-[13px] text-[#707070]">
...
const FloatingElements = () => {
  ...
  const [cookieConsent, setCookieConsent] = useState(true); // Hidden initially to avoid flash, fixed in useEffect
  ...
  return (
    <>
      {/* WhatsApp */}
      <a href="https://wa.me/916350371343" target="_blank" rel="noreferrer" className="fixed bottom-6 right-6 w-14 h-14 bg-mint rounded-full flex items-center justify-center z-[999] hover:scale-110 transition-transform shadow-lg shadow-mint/20">
